...
multilib_src_configure() {
	...
	if tc-is-cross-compiler; then
		...
		local -a CROSS_TOOLCHAIN_FLAGS_Clang_NATIVE=(
			"-DCMAKE_SYSROOT=${BROOT}"
			"-DCMAKE_C_COMPILER_TARGET="
			"-DCMAKE_CXX_COMPILER_TARGET="
			"-DCMAKE_C_FLAGS_INIT=${BUILD_CFLAGS}"
			"-DCMAKE_CXX_FLAGS_INIT=${BUILD_CXXFLAGS}"
		...
		mycmakeargs+=(
			-DLLVM_TOOLS_BINARY_DIR="${llvm_root}/bin"
			-DCLANG_TABLEGEN="${llvm_root}"/bin/clang-tblgen
			-DCROSS_TOOLCHAIN_FLAGS_Clang_NATIVE="$(local IFS=";"; echo "${CROSS_TOOLCHAIN_FLAGS_Clang_NATIVE[*]}")"
		)
	fi
